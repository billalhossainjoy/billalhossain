...
function ChatMessage({ message }: { message: Message }) {
  ...
  return (
    <div className={`flex items-end gap-2 ${isUser ? "flex-row-reverse" : ""} max-w-[85%] ${isUser ? "self-end" : "self-start"}`}>
      ...
      <p
        className={`px-4 py-2.5 rounded-xl text-sm leading-relaxed whitespace-pre-wrap break-words ${
          isUser
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: `err-${Date.now()}`,
          role: "ai",
          text: "Couldn't reach the AI. Make sure the rag-api is running.",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .chat-window { animation: chatSlideUp 0.25s cubic-bezier(0.34,1.56,0.64,1) both; }
      `}</style>
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        ...
        {open && (
          <div
            className="chat-window w-[360px] h-[520px] flex flex-col rounded-2xl border border-white/10 bg-gray-900/95 backdrop-blur-xl shadow-2xl shadow-black/50 overflow-hidden"
          >
            {/* Header */}
            ...
            {/* Suggested questions (shown only with welcome message) */}
            {messages.length === 1 && !loading && (
              <div className="px-4 pb-2 flex flex-wrap gap-2">
                {["What are his skills?", "Tell me about his projects", "How to contact him?"].map(
                  (q) => (
                    <button
                      key={q}
